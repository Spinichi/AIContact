...
export default function PhotoBook() {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (!target.closest('.calendar-btn') && !target.closest('.calendar-dropdown')) {
        setIsDropDownOpen(false);
      }
      if (!target.closest('.type-filter-btn') && !target.closest('.type-dropdown')) {
    ...
    const validFiles = files.filter(file => {
      ...
      if (file.type.startsWith('video/')) {
        ...
        if (extension && allowedVideoExtensions.includes(extension)) {
          ...
          alert(`${file.name}: 지원하지 않는 비디오 형식입니다. (mp4, mov, 3gp, mkv만 가능)`);
          return false;
        }
      }
        ...
        <div className='gallery-top-bar'>
          <div className='gallery-tabs'>
            ...
            <button
              ...
              onClick={() => { setViewMode('favorite'); setCurrentPage(0); }}
            >
              즐겨찾기
            </button>
          </div>
          <div className='gallery-actions'>
            ...
            <div className="type-filter-wrapper">
              <button
                className={`type-filter-btn ${isTypeDropDownOpen ? 'active' : ''}`}
                onClick={() => {
                ...
                <div className="photo-box" key={`right-${i}`}>
                  {thumbs[i + 12] && (
                    <img
                      ...
                      onClick={() => handleThumbnailClick(thumbs[i + 12].id, i + 12)}
                    />
                  )}
                </div>
              ))}
